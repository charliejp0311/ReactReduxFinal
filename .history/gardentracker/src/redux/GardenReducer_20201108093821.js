export default function garden (state = {
    ...
    switch (action.type) {
        ...
        case 'ADD_NOTE':
            const notesData = action.note.data
            const actionsData = action.note.included
            const actions = actionsData.map(action=>{return(action.attributes)})
            const notes = notesData.map(note=>{return(note.attributes)})
            return {
                ...state,
                notes: notes,
                actions: actions,
        ...
        case 'DELETE_NOTE':
            // debugger
            const allNotes = action.note.data;
            const notes = allNotes.map(note=>{return(note.attributes)});
            // const freshGarden = plants.filter(plant=>plant.attributes.growing ? plant.attributes : null)
