...
class GardenPlant extends Component{
    ...
    render(){
        ...
        return(
            <div>
                <h3>{plant.name}</h3>
                {plant.description ? <p>{plant.description}</p> : <form onSubmit={this.handleSubmit}><textarea name="description" value={this.state.description} onChange={this.handleChange}/><br/><input type="submit" value='Update'/><button type='delete' value='Delete'/></form>}
